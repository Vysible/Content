...
async function getMetaToken(): Promise<{ token: string; pageId: string; igId?: string }> {
  const key = await prisma.apiKey.findFirst({
    ...
    // WORDPRESS-Provider als Platzhalter bis META-Provider ergänzt wird
  })
...
export async function postInstagramDraft(caption: string, imageUrl?: string): Promise<SocialDraftResult> {
  try {
    ...
    const mediaRes = await fetch(`${GRAPH_API}/${igId}/media`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        caption,
        image_url: imageUrl ?? 'https://placehold.co/1080x1080', // Platzhalter
        access_token: token,
      }),
    })
