...
Sub-Slice A — config.ts + schema refactor:
  lib/generation/config.ts: angelegt (minPraxisQuote=0.8, minSeoQuote=0.5, ENV-Override)
  themes-schema.ts Magic Numbers: entfernt (2 Stellen: praxisPct<0.8, seoPct<0.5)
  istFrage: z.boolean().optional().default(false)
  .env.example: aktualisiert (THEMES_MIN_PRAXIS_QUOTE, THEMES_MIN_SEO_QUOTE dokumentiert)

Sub-Slice B — deterministisches istFrage + YAML:
  computeIstFrage() in themes.ts: implementiert (export, endsWith('?') || includes(keyword))
  post-parse Mapping: implementiert (parseThemesJson(rawText).map(...computeIstFrage))
  prompts/themes.yaml: istFrage entfernt, Guideline-Regel als Output-Hinweis umformuliert
  Unit-Tests: 5 neue Tests angelegt (4x computeIstFrage + 1x istFrage optional default)

TypeScript: 0 Fehler
Tests: PASS — 42/42 (war 37, +5 neue)

Commit: 23ff5ec refactor(generation): Themen-Quality-Gate Magic Numbers + istFrage deterministisch (Backlog #5)
